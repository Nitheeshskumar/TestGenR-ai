...
const getAll = async (listId) => {
  return await storage.get(listId) || [{label:'Check if button present',id:'22',isChecked:true},{label:'Check if click happns',id:'d22',isChecked:false}];
}
...
resolver.define('create', async ({ payload, context }) => {
  ...
  console.log('created',getListKeyFromContext(context), [...records, newRecord])
  return newRecord;
});
...
resolver.define('update', async ({ payload, context }) => {
  ...
  console.log('update',getListKeyFromContext(context), records)
  return payload;
});
